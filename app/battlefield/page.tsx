...
import Image from "next/image";
  ...
  async function getData() {
      const data = await prisma.battlefieldPage.findMany();
                    ...
                    <div className="flex w-full h-full flex-col justify-between">
                      ...
                      <div className="bg-background text-primary font-extrabold text-xs text-center absolute bottom-0  h-[44%] w-full ">
                        <div className="w-full">
                            ...
                            <p className="mt-1 px-2 max-w-[150px] md:max-w-[333px] break-words text-3xl text-primary line-clamp-2">{el.quantity}</p>
                        </div>     
                      </div>
                    </div>
                  </div>
