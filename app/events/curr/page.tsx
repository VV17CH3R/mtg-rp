...
async function getData() {
    const data = await prisma.adminEventPage.findFirst();
  
    return data;
  }


export default async function CurrEventPage() {
  ...
  async function postEvent(formData: FormData) {
    ...
    if(!id) {
      revalidatePath("/events/outlaws");
      return;
    }

    const event = id as string;

    await prisma.adminEventPage.update({
        where: {
            id: "32167"
        },
        data: {
...
<form action={postEvent}>
            <CardHeader>
              <CardTitle className="text-center w-full">Образ</CardTitle>
...
"
      >
        <Image
          ...
          height={600}
          alt="card"
          src={data?.imageUrl as string || "https://ncovgggrjiogvnverryj.supabase.co/storage/v1/object/public/content-for-sloweska/Sphere.jpg"}
        />
